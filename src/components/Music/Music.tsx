...
import { v1 } from "uuid";
import { AddMusicAC, musicpageType } from "../../redux/music-reducer";
import s from './Music.module.css';
...
export class Music extends React.Component<musicPropsType> {
    ...
    componentDidMount() {
        ...
        if (this.props.musicpage.items.length <= 1) {
            axios
                .request(options).then(function (responce: any) {
                    console.log(responce.data.items);
                    responce.data.items.map((m: musicElemType) =>
                        AddMusicAC(m.track.name, m.track.preview_url))
                })
                .catch(function (error: any) {
                    console.error(error);
                });
        }
        let musicElem = this.props.musicpage.items.map(m => (
            <div key={v1()}>
                <a
                    key={v1()}
                    target="_blank"
